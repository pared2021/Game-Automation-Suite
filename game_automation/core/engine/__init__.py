...
class GameEngine:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Initialize device manager with simulation mode if no real device support
        simulation_mode = config.get('device', {}).get('simulation_mode', True)
        self.device_manager = DeviceManager(simulation_mode=simulation_mode)
    ...
    async def initialize(self) -> None:
        ...
        try:
            ...
            detailed_logger.warning(f"Device initialization failed: {str(e)}. Running in simulation mode.")
            self.device_manager.simulation_mode = True
            await self.device_manager.connect()
